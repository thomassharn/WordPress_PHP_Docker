...
class Utilities {
	...
	public static function processResponse( $currentURL, $certFingerprint, $signatureData, SAML2_Response $response, $certNumber, $relayState ) {
		...
		if ($sessionNotOnOrAfter !== NULL && $sessionNotOnOrAfter <= time() - 60) {
			$Logger->add_log('Received an assertion with a session that has expired. Check clock synchronization on IdP and SP.',$Logger::ERROR);
            $error_code = mo_saml_options_enum_error_codes::$error_codes['WPSAMLERR008'];
            self::mo_saml_die($error_code);
        }
        ...
        if(strcmp($issuerToValidateAgainst, $issuer) === 0) {
            if(!empty($audiences)) {
                if(in_array($spEntityId, $audiences, TRUE)) {
                    ...
                    $saml_logger->add_log(mo_saml_error_log::showMessage('UTILITIES_INVALID_AUDIENCE_URI',array('spEntityId'=>$spEntityId,'audiences'=>$audiences)), MoSAMLLogger::ERROR);
            ...
            $saml_logger->add_log(mo_saml_error_log::showMessage('UTILITIES_INVALID_ISSUER', array('issuerToValidateAgainst' => $issuerToValidateAgainst, 'issuer' => $issuer)), MoSAMLLogger::ERROR);
    ...
    public static function mo_saml_die( $error_code ) {
        wp_die("We could not sign you in. Please contact your administrator with the following error code.<br><br>Error code: <b>".$error_code['code']."</b>","Error: ".$error_code['code']);
    }
